  ...
  readFile(files: FileList) {
    if (files) {
      Array.from(files).forEach((file: File) => {
        ...
        reader.onload = e => {
          if (
            !this.selectedFiles.find(({ base64 }) => base64 === e.target.result)
          ) {
            this.selectedFiles = [
              ...this.selectedFiles,
  ...
  renderPlaceholder(type, filename, index) {
    return (
      <div class="item-preview">
        <img
          class="icon"
          src={this[`selected${type}Placeholder`]}
          alt={filename}
        />
        <p>{filename}</p>
        <button type="button" onClick={() => this.removeFile(index)}>
    ...
    return this.modalVisible ? (
      <div class="modal" onClick={this.toggleModal.bind(this)}>
        ...
        >
          ...
          >
            <svg viewBox="0 0 32 32" id="icon-close" width="100%" height="100%">
              <path d="M10.06 7.94a1.5 1.5 0 0 0-2.12 2.12L13.878 16l-5.94 5.94a1.5 1.5 0 0 0 2.122 2.12L16 18.122l5.94 5.94a1.5 1.5 0 0 0 2.12-2.122L18.122 16l5.94-5.94a1.5 1.5 0 0 0-2.122-2.12L16 13.878l-5.94-5.94z" />
            </svg>
          </button>
